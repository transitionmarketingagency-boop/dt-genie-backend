...
// Chat endpoint with enhanced context retrieval (Gemini 2.5 Flash)
app.post("/api/chat", chatRateLimiter, async (req, res) => {
  try {
    ...
    // Build a human readable history snippet for the prompt (last N messages)
    const historyForPrompt = history
      .slice(-10)
      .map(h => `${h.role === "assistant" ? "Assistant" : "User"}: ${h.content}`)
      .join("\n");

    // Final prompt: system + known context + short history + user message
    ...
    const finalPrompt = finalPromptParts.filter(Boolean).join("\n");

    // Format history for askGemini if helper accepts a structured history (pass both)
    const formattedHistory = history.slice(-10).map(h => ({
      role: h.role === "assistant" ? "assistant" : "user",
      content: h.content,
    }));

    // Call Gemini via your helper (expects askGemini(messageOrPrompt, history?))
    const aiReply = await askGemini(finalPrompt, formattedHistory);
    ...
    const replyEmbedding = await generateEmbedding(aiReply);
    memoryStore.addEntry(sessionId, aiReply, "memory", replyEmbedding);
