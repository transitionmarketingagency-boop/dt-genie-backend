...
import bookingFlow from "../bookingFlow.js"; // 🔥 Dynamic booking integration
...
export async function generateHybridResponse({ message, sessionId, history = [] }: HybridRequest): Promise<string> {
  try {
    ...
    if (brain.type === "greeting") {
      ...
      const greetedAlready = previous.some((m: any) => m.role === "assistant" && m.content?.includes("Hello"));
    ...
    try { historyMessages = await memoryService.getRecentContext(sessionId); } catch { historyMessages = history.slice(-6); }
    ...
    const vectorText = compressContext([...strategicChunks, { text: vector.text }]);
    const vectorCount = strategicChunks.length + vector.count;

    /* ---------- STAGE INSTRUCTION ---------- */
    let stageInstruction = "";
    if (brainContext.stage === "discovery") stageInstruction = "Ask clarifying questions about the user's business.";
    if (brainContext.stage === "strategy") stageInstruction = "Provide strategic insights for business growth.";
    if (brainContext.stage === "service") stageInstruction = "Explain relevant services clearly.";
    if (brainContext.stage === "conversion") stageInstruction = "Encourage booking a consultation.";

    /* ---------- SERVICE RECOMMENDATION ---------- */
    const recommendedServiceText = brainContext.recommendedService ? `Recommended service focus: ${brainContext.recommendedService}` : "";

    /* ---------- BOOKING SIGNAL ---------- */
    const bookingSignal = brainContext.triggerBooking ? "If appropriate, encourage the user to schedule a consultation." : "";
...
`;
    ...
    console.log(`[Hybrid RAG] Model=${modelUsed} | Stage=${brainContext.stage} | Intents=${detectedIntentNames.join(",")} | Chunks=${vectorCount} | Service=${detectedService ?? "none"} | LeadScore=${brainContext.leadScore} | ResponseLen=${response.length}`);
