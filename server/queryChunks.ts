...
/* ================= BLOCKED PLATFORMS & TOOLS ================= */
const blockedPlatforms = [
  "midjourney", "runway", "pika", "kling", "openai",
  "anthropic", "replicate", "stability ai",
  "creatify", "wisepops", "adcreative.ai", "aiclicks.io"
];
  ...
  try {
    ...
    cachedChunks = parsed
      ...
      .map((c: any) => {
        ...
        let embedding = c.embedding.map(Number).filter(n => !Number.isNaN(n));
        embedding = normalizeVector(embedding);
        return {
          text: cleanText,
          source: typeof c.source === "string" && c.source.trim() ? c.source : "Digital Transition Marketing",
          embedding,
    ...
    cachedChunks = cachedChunks.filter(c => c.embedding.length === embeddingSize);
...
export async function getTopChunks(
  ...
  for (const q of queries) {
    ...
    if (embedding.length === embeddingSize) allEmbeddings.push(embedding);
  }
  ...
  const scored = cachedChunks.map(chunk => {
    ...
    const hybridScore = bestScore * 0.82 + keywordBoost * 0.13 + intentBoost;
  ...
  console.log(`🔎 Vector search | Query="${normalized.slice(0, 40)}" | Results=${results.length}`);
